...
#include <iostream>
#include <numeric>
AntSystem::AntSystem(const char * nodeFileName, const char * edgeFileName, const int numberOfAnts, const float alpha, const float beta, const float targetPathLength)
  : graph(nodeFileName, edgeFileName), numberOfAnts_(numberOfAnts), alpha_(alpha), beta_(beta), targetPathLength_(targetPathLength) {
  InitPheromoneLevels();
};
...
void AntSystem::Run(){
  ...
  while(minPathLength > targetPathLength_ && iIteration < 100){
    iIteration++;
